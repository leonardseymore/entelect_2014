...
public class MoveToHighestClusterPill extends Task {

    public static final int DEFAULT_NUM_MATCHES = 10;
    private boolean tactical;
    private int numMatches;

    public MoveToHighestClusterPill() {
        this(false, DEFAULT_NUM_MATCHES);
    }

    public MoveToHighestClusterPill(boolean tactical, int numMatches) {
    ...
    public boolean run(PacmanAgent pacmanAgent, GameState gameState) {
        ...
        for (SearchNode node : matches) {
            float potential = gameState.getInfluenceMap().getPillCluster()[node.pos.x][node.pos.y];
